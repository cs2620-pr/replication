...
class MessageService(pb2_grpc.MessageServiceServicer):
    ...
    def _set_value(self, key: str, value: Any) -> bool:
        ...
        try:
            ...
            if hasattr(self.raft_node, 'apply_to_state_machine'):
                ...
                self.raft_node.apply_to_state_machine(command)
                return True
            ...
            # Fallback: Create a simple client to call the leader
                ...
                for node in self.raft_node.nodes:
                    try:
                        node_id, host, port = node.get('id'), node.get('host'), node.get('port')
    ...
    def _get_user_by_username(self, username: str) -> Tuple[Optional[str], Optional[Dict]]:
    ...
    def Register(self, request, context):
        ...
        if not username or not password:
            return pb2.RegisterResponse(success=False, message="Username and password are required")
        
        # Check if username already exists
        user_id, _ = self._get_user_by_username(username)
        if user_id:
            return pb2.RegisterResponse(success=False, message="Username already exists")
        ...
        if self._set_value("users", users):
            return pb2.RegisterResponse(success=True, message="Registration successful", user_id=user_id)
        else:
            return pb2.RegisterResponse(success=False, message="Failed to register user")
    ...
    def Login(self, request, context):
        ...
        # Validate input
        if not username or not password:
            return pb2.LoginResponse(success=False, message="Username and password are required")
        
        # Find user
        user_id, user_data = self._get_user_by_username(username)
        if not user_id or not user_data:
            return pb2.LoginResponse(success=False, message="Invalid username or password")
    ...
    def DeleteAccount(self, request, context):
        ...
        if not user_data or user_data.get("password_hash") != self._hash_password(password):
        ...
        return pb2.DeleteAccountResponse(success=True, message="Account deleted successfully")
    ...
    def SendMessage(self, request, context):
        ...
        # Validate input
        if not content:
            return pb2.SendMessageResponse(success=False, message="Message content cannot be empty")
        
        # Verify receiver exists
        receiver = self._get_user_by_id(receiver_id)
        if not receiver:
            return pb2.SendMessageResponse(success=False, message="Recipient does not exist")
        ...
        if self._set_value("messages", messages):
            ...
            return pb2.SendMessageResponse(success=False, message="Failed to send message")
    ...
    def GetMessages(self, request, context):
        ...
        before_timestamp = request.before_timestamp or int(time.time() * 1000)  # Default to current time
        ...
        for msg_id, msg in all_messages.items():
            ...
            is_sender = msg.get("sender_id") == user_id and msg.get("receiver_id") == other_user_id
            is_receiver = msg.get("sender_id") == other_user_id and msg.get("receiver_id") == user_id
            
            if (is_sender or is_receiver) and msg.get("timestamp", 0) < before_timestamp:
        ...
        pb_messages = []
        for msg in conversation_messages:
            pb_messages.append(pb2.Message(
    ...
    def DeleteMessage(self, request, context):
        ...
        if message.get("sender_id") != user_id:
            return pb2.DeleteMessageResponse(success=False, message="Not authorized to delete this message")
        ...
        if self._set_value("messages", messages):
            ...
            return pb2.DeleteMessageResponse(success=False, message="Failed to delete message")
    ...
    def MarkMessageRead(self, request, context):
        ...
        if not message:
            return pb2.MarkMessageReadResponse(success=False, message="Message not found")
        
        # Verify user is the recipient
        if message.get("receiver_id") != user_id:
            return pb2.MarkMessageReadResponse(success=False, message="Not authorized to mark this message as read")
        ...
        # Save to Raft state machine
        if self._set_value("messages", messages):
            return pb2.MarkMessageReadResponse(success=True, message="Message marked as read")
        else:
            return pb2.MarkMessageReadResponse(success=False, message="Failed to mark message as read")
    ...
    def GetUnreadCounts(self, request, context):
        ...
        for sender_id, count in unread_counts.items():
            pb_unread_counts.append(pb2.UserUnreadCount(
                user_id=sender_id,
                count=count
            ))
